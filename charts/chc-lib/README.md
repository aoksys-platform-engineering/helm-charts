...
![Version: 0.9.9](https://img.shields.io/badge/Version-0.9.9-informational?style=flat-square) ![Type: library](https://img.shields.io/badge/Type-library-informational?style=flat-square)
...
## Common values
Values that are used across various templates and can be set at ``.Values.<Value>``.
...
| podSecurityContext | string | `"default"` | Security context to add to all pods. Can be freely defined as a dict, or a preset name can be supplied as a string. |
...
| affinity.podAffinity | object | `{}` | Pod affinity to set for pod scheduling. Useful to ensure that pod replicas are placed on the same node. Can be freely defined as dict, or as string from predefined preset names (``soft`` or ``hard``). See 'Pod affinity presets' section in README for more. |
| affinity.podAntiAffinity | string | `"soft"` | Pod anti affinity to set for pod scheduling. Useful to ensure that pod replicas are placed on different nodes. Can be freely defined as dict, or as string from predefined preset names (``soft`` or ``hard``). See 'Pod affinity presets' section in README for more. |
| affinity.nodeAffinity | object | `{}` | Node affinity to set for pod scheduling. Useful to ensure that pods are placed on a specific node.# Has to be supplied as dict, because no presets exists for it. |
...
| sidecarContainers | object | `{}` | Sidecar containers to inject into the pod. Setting values for "sidecarContainers" works the same way as configuring initContainers, with the exception of not needing to set a "weight" value. See 'InitContainers' section in README for more. |
...
``.Values.controller.affinity.podAntiAffinity`` stanzas for the controller, because values for them can be provided as dict or from predefined strings (``soft`` or ``hard``).
...
When providing a preset name as string, a predefined YAML block will be injected into the template.

You can choose between a ``soft`` and ``hard`` preset:
...
```yaml
# using the "soft" preset for podAntiAffinity ...
---
...
controller:
  ...
...
# ... results in the following YAML block:
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      affinity:
        podAntiAffinity:
          # would be substituted with a "requiredDuringSchedulingIgnoredDuringExecution" block when using the "hard" preset
          preferredDuringSchedulingIgnoredDuringExecution:
            - weight: 1
              podAffinityTerm:
                ...
                labelSelector:
                  matchLabels:
                    app.kubernetes.io/instance: chc-lib-unittests
                    app.kubernetes.io/name: chc-lib-unittests
...
```

```yaml
# a freely defined podAntiAffinity is pasted as-is into the template
---
...
controller:
  ...
...
This section provides example values and explains the usage of the ``.Values.controller.initContainers`` and
``.Values.controller.sidecarContainers`` stanzas because default values are set to an empty dict.
...
controller:
  ...
  initContainers:
    ...
    liquibase:
      ...
      # all key/values go through tpl, so you can add all fields valid for an initContainer
      env:
...
Setting values for ``sidecarContainers`` works the same way as for initContainers with the exception that
sidecarContainers don't need to guarantee an order of execution and therefore don't require a ``weight`` value.
Setting the ``weight`` value for a sidecarContainer will result in a schema violation, which prevents the installation
to succeed.
...
| Value | Type | Default | Description |
|-------|------|-------------|---------|
| volumeClaimTemplates | list | `[]` | List of volumeClaimTemplates for the pod. Values go through tpl. |
...
| minReplicas | int | `1` | Sets the "minReplicas" of pods managed by the hpa. |
| maxReplicas | int | `3` | Sets the "maxReplicas" of pods managed by the hpa. |
